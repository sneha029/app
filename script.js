...
/* ========== PROFILE ========== */
function openProfile(){ document.getElementById('profileModal').classList.add('show'); document.getElementById('profileName').value = profile.name || ''; document.getElementById('profileAvatarImg').src = profile.avatar || ''; }
function closeProfile(){ document.getElementById('profileModal').classList.remove('show'); }
function uploadAvatar(e){ const f = e.target.files[0]; if(!f) return; const r = new FileReader(); r.onload = ()=>{ profile.avatar = r.result; document.getElementById('profileAvatarImg').src = r.result; document.getElementById('hdrAvatarImg').src = r.result; localStorage.setItem('mw_profile', JSON.stringify(profile)); toast('Avatar updated','success'); }; r.readAsDataURL(f); }
function saveProfile(){ profile.name = document.getElementById('profileName').value || 'Guest'; localStorage.setItem('mw_profile', JSON.stringify(profile)); document.getElementById('hdrName').innerText = profile.name; document.getElementById('hdrAvatarImg').src = profile.avatar || ''; toast('Profile saved','success'); closeProfile(); }
function resetProfile(){ profile = {}; localStorage.removeItem('mw_profile'); document.getElementById('profileName').value=''; document.getElementById('profileAvatarImg').src=''; document.getElementById('hdrAvatarImg').src=''; document.getElementById('hdrName').innerText='Guest'; toast('Profile reset'); }
